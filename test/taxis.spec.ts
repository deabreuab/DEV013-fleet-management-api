...
describe('GET /taxis', () => {
    test('It should respond with a status code 200', async () => {
        const response = await request(app).get('/api/taxis').send()
    ...
    test('It should respond with 10 taxis as result', async () => {
        const response = await request(app).get('/api/taxis').send()
        console.log('LENGTH DE MI DATA', response.body.data.length)
        expect(response.body.data).toHaveLength(10)
    })

    test('It should respond with 4 taxis as result', async () => {
        const response = await request(app).get('/api/taxis?limit=4').send()
        console.log('LENGTH DE MI DATA', response.body.data.length)
...
describe('POST /taxis', () => {
    test('It should respond with a status code 201', async () => {
        const response = await request(app).post('/api/taxis').send({
            id: 12345,
            plate: '123-ABCD'
        })
    ...
    test('It should respond with a status code 400', async () => {
        ...
        expect(response.body.message).toBe('You have not provided a valid plate for the taxi. Please check it out trying to register a new Taxi')
    })
})
...
describe('GET /taxis/taxiId', () => {
    test('It should respond with a status code 200', async () => {
        const taxiId = 12345
        const response = await request(app).get(`/api/taxis/${taxiId}`).send()
        console.log('RESPONDE TAXIID', response.body)
        expect(response.status).toBe(200)
    ...
    test('It should respond with a status code 404 when taxi id isn`t found', async () => {
        const response = await request(app).get('/api/taxis/3').send()
...
describe('PATCH /taxis', () => {
    test('It should respond with a status code 200 when update a register', async () => {
        const response = await request(app).patch('/api/taxis/12345').send({
            id: 123456,
            plate: '123-ABCD'
        })
        console.log('MI BODY RESPONSE', response.body)
        expect(response.status).toBe(200)
...
describe('DELETE /taxis', () => {
    test('It should respond with a status code 200 when delete a register', async () => {
        const response = await request(app).delete('/api/taxis/123456').send()
        expect(response.status).toBe(200)
